...
create table covid.covidcast_names(csv_file text primary key, staging_table text not null, signal text not null);
...
begin
  insert into covid.covidcast_names(csv_file, staging_table, signal) values
    (dir||'covidcast-fb-survey-smoothed-'|| 'wearing-mask' ||'-2020-09-13-to-2020-11-01.csv', 'mask_wearers',   'smoothed_wearing_mask'),
    (dir||'covidcast-fb-survey-smoothed-'|| 'cli'          ||'-2020-09-13-to-2020-11-01.csv', 'symptoms',       'smoothed_cli'),
    (dir||'covidcast-fb-survey-smoothed-'|| 'hh-cmnty-cli' ||'-2020-09-13-to-2020-11-01.csv', 'cmnty_symptoms', 'smoothed_hh_cmnty_cli');
  end;
$body$;

create unique index covidcast_names_staging_table_unq on covid.covidcast_names(staging_table);
create unique index covidcast_names_signal_unq on covid.covidcast_names(signal);
...
select covid.cr_copy_from_scripts(1);
\o
\i /tmp/copy_from_csv.sql

\o /tmp/copy_from_csv.sql
select covid.cr_copy_from_scripts(2);
\o
\i /tmp/copy_from_csv.sql

\o /tmp/copy_from_csv.sql
select covid.cr_copy_from_scripts(3);
\o
\i /tmp/copy_from_csv.sql
