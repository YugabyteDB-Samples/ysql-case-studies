...
call mgr.grant_priv(   'usage', 'type', 'code.m_and_ds', 'client');
...
exception
  ...
  when foreign_key_violation then
    ...
    outcome := 'User error: cannot delete a master''s last surviving detail.'||e'\n'||
               '            '||detail;
end;
$body$;
call mgr.revoke_all_from_public('procedure', 'code.delete_specified_details(text, text[])');
call mgr.grant_priv( 'execute', 'procedure', 'code.delete_specified_details(text, text[])', 'client');
