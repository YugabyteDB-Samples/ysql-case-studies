    ...
    when true then
      ...
      exception when undefined_table then
        ...
        if msg != 'relation "pg_temp.cascade_delete_flag" does not exist' then
          ...
          create temp table pg_temp.cascade_delete_flag(val boolean not null) on commit delete rows;
          insert into pg_temp.cascade_delete_flag(val) values(true);
        end if;
      end;
  end case;
...
$body$;
revoke execute on procedure data.set_cascade_delete_flag(boolean) from public;
...
$body$;
revoke execute on function data.cascade_delete_flag() from public;
...
as $body$
declare
  level constant text not null := current_setting('transaction_isolation');
begin
...
$body$;
revoke execute on procedure data.assert_iso_lvl_is_serializable() from public;
--------------------------------------------------------------------------------
/*
...
$body$;
revoke execute on function data.enforce_masters_mk_immutable() from public;
...
$body$;
revoke execute on function data.enforce_details_dk_mk_immutable() from public;
...
$body$;
revoke execute on function data.call_set_cascade_delete_flag() from public;
...
$body$;
revoke execute on function data.enforce_mdry_1_to_m_rule_for_masters() from public;
...
$body$;
revoke execute on function data.enforce_mdry_1_to_m_rule_for_details() from public;
