create function mgr.rule_off(t in text, mode in text = 'caption')
...
as $body$
declare
  r constant text not null := rpad('=', 130, '=');
begin
  case mode
    when 'caption' then
      z := r;                                   return next;
...
grant execute on function mgr.rule_off(text, text) to public;
----------------------------------------------------------------------------------------------------

create function mgr.fmt(n in numeric, template in text)
  returns text
  stable
  security invoker
  set search_path = pg_catalog, pg_temp
...
grant execute on function mgr.fmt(numeric, text) to public;
----------------------------------------------------------------------------------------------------

create function mgr.fmt(i in int, template in text)
  returns text
  stable
  security invoker
  set search_path = pg_catalog, pg_temp
...
grant execute on function mgr.fmt(int, text) to public;
----------------------------------------------------------------------------------------------------

create function mgr.duration_as_text(t in numeric)
  returns text
  stable
  security invoker
  set search_path = pg_catalog, pg_temp
...
begin
  case
    ...
    when t >= confidence_limit and t < ms_limit then
      result := mgr.fmt(t*ms_pr_sec, '9999')||' ms';

    when t >= ms_limit and t < cs_limit then
      result := mgr.fmt(t, '90.99')||' ss';

    when t >= cs_limit and t < secs_pr_min then
      result := mgr.fmt(t, '99.9')||' ss';
    ...
    when t >= secs_pr_min and t < secs_pr_hour then
      ...
      begin
        result := mgr.fmt(mins, '09')||':'||fmt(secs, '09')||' mi:ss';
      end;
    ...
    when t >= secs_pr_hour and t < secs_pr_day then
      ...
      begin
        result := mgr.fmt(hours, '09')||':'||fmt(mins,  '09')||' hh:mi';
      end;
    ...
    when t >= secs_pr_day then
      declare
        ...
        mins   constant int     not null := round(mi - hours*mins_pr_hour);
      begin
        result := mgr.fmt(days,  '99')||' days '||
                  mgr.fmt(hours, '09')||':'||fmt(mins,  '09')||' hh:mi';
      end;
  end case;
  return result;
end;
...
grant execute on function mgr.duration_as_text(numeric) to public;
----------------------------------------------------------------------------------------------------

create procedure mgr.start_stopwatch()
  security invoker
  set search_path = pg_catalog, pg_temp
  language plpgsql
as $body$
...
grant execute on procedure mgr.start_stopwatch() to public;
----------------------------------------------------------------------------------------------------

create function mgr.stopwatch_reading_as_dp()
...
grant execute on function mgr.stopwatch_reading_as_dp() to public;
----------------------------------------------------------------------------------------------------

create function mgr.stopwatch_reading()
  returns text
...
as $body$
declare
  t constant text not null := mgr.duration_as_text(stopwatch_reading_as_dp()::numeric);
begin
  return t;
end;
$body$;

grant execute on function mgr.stopwatch_reading() to public;
----------------------------------------------------------------------------------------------------

create function mgr.stopwatch_reading(start_time in double precision)
  returns text
  volatile
  security invoker
  set search_path = pg_catalog, pg_temp
...
declare
  ...
  diff       constant double precision not null := curr_time - start_time;
begin
  return mgr.duration_as_text(diff::numeric);
end;
$body$;

grant execute on function mgr.stopwatch_reading(double precision) to public;
