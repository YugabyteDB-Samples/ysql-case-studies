...
declare
  ...
  v_message                 text;
begin
  if not mgr.is_good_name(nickname) then assert false,
    'By convention for the YBMT cluster, "cr_role()" and its cousins do not allow'||e'\n'||
    'values for "nickname" that contain any characters other than "a-z", "0-9", and underscores.';
  end if;
...
begin
  ...
  case
    when not (v_name_exists and v_ok) then
      ...
      execute format('alter role %I with password %L', r_name::text, password);
  end case;
end;
$body$;
