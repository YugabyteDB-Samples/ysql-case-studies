...
\t on \\ select rule_off('The "approved_timezone_names" view', 'level_2'); \t off

\t on \\ select rule_off('distinct offsets from "approved_timezone_names"', 'level_3'); \t off
...
\t on \\ select rule_off('timezones where offset isn''t an integral no. of hours', 'level_3'); \t off
with
  v as (
