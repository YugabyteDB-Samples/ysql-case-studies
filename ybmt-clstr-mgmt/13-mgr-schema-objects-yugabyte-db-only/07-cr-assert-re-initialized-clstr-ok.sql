create type mgr.rt as (name text, super boolean, createrole boolean, createdb boolean, canlogin boolean, connlimit int, config text[]);
...
declare
  ...
  r                       mgr.rt   not null := ('', false, false, false, false, 0, '{}'::text[])::mgr.rt;
  expected_roles constant mgr.rt[]          := (
      select array_agg((rolname, rolsuper, rolcreaterole, rolcreatedb, rolcanlogin, rolconnlimit, rolconfig)::mgr.rt)
      from pg_roles
      where rolname  !~ '^pg_'
      and   rolname  !~ '^yb_'
      and   rolinherit
...
begin
  ----------------------------------------------------------------------------------------------------------------------
  -- Check the roles.
  foreach r in array expected_roles loop
    case r.name
      when 'postgres' then
        ...
        */
        ...
        assert (r.config)[1] = 'search_path=dt_utils, mgr, extensions, pg_catalog, pg_temp',
      ...
      when 'clstr$mgr' then
        ...
        assert false,
          'Unexpected: non-system role other than "postgres", "yugabyte", or "clstr$mgr" found: '||
          r.name;
    end case;
  end loop;
  ...
  foreach d in array dbs loop
    case d.name
      when 'yugabyte' then
        ...
        /*
          The fact that the grantor of "connect" and "create" for "clstr$mgr" is nominaly "posrgres"
          is of no consequence. The grants were actually done by "yugabyte". But "postgres" is the owner
         of the "yugabyte" database.
        */
        assert (d.acl)[1] = '"clstr$mgr"=Cc/postgres',
          'Unexpected: database "yugabyte" should allow "connect" and "create" for "clstr$mgr": '||(d.acl)[1];
