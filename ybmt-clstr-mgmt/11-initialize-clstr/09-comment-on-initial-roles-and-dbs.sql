...
declare
  ...
  role_clstr$developer_comment constant text not null :=
    'Pure role (cannot log in). Will be granted to the non-client roles in '    ||e'\n'||
    'each tenant database to bring "select" access to the views and tables '    ||e'\n'||
    'in the "pg_catalog" and "information_schema" schemas and to all objects '  ||e'\n'||
    'in the "mgr", "dt_utils", and "extensions" schemas. '                      ||e'\n';
begin
  /*
  ...
  */
  ...
  execute format('comment on role     clstr$developer    is %L', role_clstr$developer_comment);
end;
$body$;
