...
begin
  case
    ...
    when t >= secs_pr_min and t < secs_pr_hour then
      ...
      begin
        result := mgr.fmt(mins, '09')||':'||mgr.fmt(secs, '09')||' mi:ss';
      end;
    ...
    when t >= secs_pr_hour and t < secs_pr_day then
      ...
      begin
        result := mgr.fmt(hours, '09')||':'||mgr.fmt(mins,  '09')||' hh:mi';
      end;
    ...
    when t >= secs_pr_day then
      ...
      begin
        result := mgr.fmt(days,  '99')||' days '||
                  mgr.fmt(hours, '09')||':'||mgr.fmt(mins,  '09')||' hh:mi';
      end;
  end case;
  return result;
end;
...
as $body$
declare
  t constant text not null := mgr.duration_as_text(mgr.stopwatch_reading_as_dp()::numeric);
begin
  return t;
end;
$body$;
