create function mgr.rule_off(t in text, mode in text = 'level_1')
...
create function mgr.rule_off(lines in text[])
  returns table(z text)
  immutable
  security invoker
  set search_path = pg_catalog, pg_temp
...
grant execute on function mgr.rule_off(text[]) to public;
